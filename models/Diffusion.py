...
class Diffusion(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.timesteps = args.cd_alpha
        self.local_lambda = args.ll
        self.diff_noise = args.cdl
        self.align_lambda = args.al
        self.n_division = args.n_hh
        ...
        if self.n_division:
            self.d1_hh = [Division(args, self.shared_dim, self.n_division) for _ in range(self.n_layers+1)]
            self.d1_hh = nn.ModuleList(self.d1_hh)

            self.d2_hh = [Division(args, self.shared_dim, self.n_division) for _ in range(self.n_layers+1)]
    ...
    def get_diffused_items(self, item_emb, item_pos_ids, item_neg_ids):
    ...
    def get_cd_loss(self, ori_users, ori_items, tar_users, tar_items):
        ...
        c1_final_scores = self.timesteps * (neg_scores-c1_scores) + (1-self.timesteps) * (c1_scores-pos_scores)
        c2_final_scores = self.timesteps * (neg_scores-c2_scores) + (1-self.timesteps) * (c2_scores-pos_scores)
    ...
    def calculate_loss(self, inter_d1, neg_d1_item, inter_d2, neg_d2_item):
        ...
        for i in range(self.neg_ratio):
            ...
            # local diffused samples
            if self.local_lambda:
                d1_c1, d1_c2 = self.get_diffused_items(d1_item_emb, inter_d1[:, 1], neg_d1_item[:, i])
                ...
                d2_c1, d2_c2 = self.get_diffused_items(d2_item_emb, inter_d2[:, 1], neg_d2_item[:, i])
            ...
            if self.diff_noise:
                ...
                d1_loss = d1_loss + self.diff_noise * d1_cd_loss

                d2_cd_loss = self.get_cd_loss(d2_cd_users, d2_cd_items, d1_cd_users, d1_cd_neg_items)
                d2_loss = d2_loss + self.diff_noise * d2_cd_loss
